...
class DbService {
	...
	async getStudentsByGroup(groupId) {
		try {
			const response = await new Promise((resolve, reject) => {
				const query = "SELECT DISTINCT Etudiant.Nom, Etudiant.Prenom FROM Etudiant, Groupe, Dossier, Seance_restant, Matiere WHERE Groupe.Id_Group = ? AND Dossier.IdEtudiant = Etudiant.IdEtudiant AND Dossier.numDossier = Seance_restant.numDossier AND Groupe.Id_Matier = Matiere.id_Matier";
	...
	async addToInactiveGroup(seance) {
		try {
			let group = await new Promise((resolve, reject) => {
				const query = "SELECT Id_Group FROM Groupe WHERE Statut = 'Inactive' and Id_Matier = ?";
				connection.query(query, [seance.Id_Matier], (err, result) => {
					if (err) reject(err);
					resolve(result[0]);
				});
			});
			...
			for (let i = 0; i < days.length; i++) {
				for (let j = 1; j <= 6; j++) {
					for (let k = 1; k <= 2; k++) {
						...
						if (rows.length === 0) {
							result.push({ Jour_seance: days[i], numSalle: k, Num_seance: j });
						}
					}
				}
			}
